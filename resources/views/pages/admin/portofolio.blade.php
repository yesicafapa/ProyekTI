...
<div class="mx-auto max-w-screen-2xl p-4 md:p-6" 
     x-data="{ 
        ...
        actionUrl: '',
        imageUrl: '',
        {{-- formData disesuaikan dengan kolom database portofolio --}}
        ...
        openEdit(item) {
            ...
            {{-- Menggunakan item.gambar sesuai database --}}
        ...
        <table class="w-full text-left">
            <thead>
                <tr class="bg-slate-50/50 border-b border-slate-200 dark:bg-slate-800/30 dark:border-slate-800">
                    <th class="px-8 py-5 text-xs font-bold uppercase tracking-widest text-slate-400">Project</th>
                    <th class="px-8 py-5 text-center text-xs font-bold uppercase tracking-widest text-slate-400">Status</th>
                    <th class="px-8 py-5 text-right text-xs font-bold uppercase tracking-widest text-slate-400">Aksi</th>
                </tr>
            </thead>
                ...
                <tr class="group hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-all">
                    <td class="px-8 py-5">
                        <div class="flex items-center gap-5">
                            <div class="h-14 w-20 flex-shrink-0 overflow-hidden rounded-xl border border-slate-200 shadow-sm dark:border-slate-700">
                                <img src="{{ $item->gambar ? asset('storage/' . $item->gambar) : asset('assets/img/no-image.png') }}" class="h-full w-full object-cover group-hover:scale-110 transition-transform duration-500">
                            </div>
                            <div>
                                <div class="text-base font-bold text-slate-800 dark:text-slate-100 group-hover:text-orange-500 transition-colors">{{ $item->judul }}</div>
                                <div class="mt-1 flex items-center gap-2 text-xs font-medium text-slate-400">
                                    <span class="text-blue-500/80">Admin</span>
                                    <span>•</span>
                                    <a href="{{ $item->url }}" target="_blank" class="hover:text-orange-500 underline truncate max-w-[200px]">{{ $item->url }}</a>
                                </div>
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="px-8 py-5 text-right">
                        <div class="flex justify-end gap-2">
                            {{-- Tombol Edit memanggil openEdit dengan data item --}}
                            ...
                            <form action="{{ route('portofolio.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus portofolio ini?')">
                                @csrf @method('DELETE')
    ...
    {{-- PEMANGGILAN MODAL KHUSUS PORTOFOLIO --}}
