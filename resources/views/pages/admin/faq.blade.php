...
<div class="mx-auto max-w-screen-2xl p-4 md:p-6" 
    ...
    <div class="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            ...
            <p class="text-slate-500 dark:text-slate-400 font-medium mt-1">Kelola pertanyaan umum pelanggan</p>
        </div>
        ...
        <table class="w-full text-left">
            <thead>
                <tr class="bg-slate-50/50 border-b border-slate-200 dark:bg-slate-800/30 dark:border-slate-800">
                    <th class="px-8 py-5 text-xs font-bold uppercase tracking-widest text-slate-400">Tanya & Jawab</th>
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                ...
                <tr class="group hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-all">
                    <td class="px-8 py-5">
                        <div class="text-base font-bold text-slate-800 dark:text-slate-100">{{ $item->pertanyaan }}</div>
                        <div class="mt-1 text-sm text-slate-400 line-clamp-1 italic">"{{ $item->jawaban }}"</div>
                    </td>
                    ...
                    <td class="px-8 py-5 text-right">
                        <div class="flex justify-end gap-2">
                            ...
                            <form action="{{ route('faq.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus FAQ ini?')">
                                @csrf @method('DELETE')
