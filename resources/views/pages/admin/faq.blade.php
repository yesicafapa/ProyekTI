...
<div class="mx-auto max-w-screen-2xl p-4 md:p-6" 
    ...
    <div class="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            <h2 class="text-3xl font-extrabold tracking-tight text-slate-800 dark:text-white">Manajemen FAQ</h2>
            <p class="text-slate-500 dark:text-slate-400 font-medium mt-1">Kelola pertanyaan umum pelanggan CV SEOVDETECH</p>
        </div>
            ...
            <table class="w-full min-w-[800px] text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100 dark:bg-white/[0.02] dark:border-white/5">
                        <th class="px-10 py-7 text-[11px] font-bold uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">TANYA & JAWAB</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-white/[0.03]">
                    ...
                    <tr class="group hover:bg-slate-50/50 transition-all duration-300 dark:hover:bg-white/[0.01]">
                        <td class="px-10 py-6">
                            <div class="flex flex-col gap-1.5">
                                ...
                                <div class="text-[13px] font-medium text-slate-500 dark:text-slate-400 italic line-clamp-1 max-w-3xl">
                                    "{{ $item->jawaban }}"
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-10 py-6 text-right">
                            <div class="flex justify-end gap-3">
                                ...
                                <form action="{{ route('faq.destroy', $item->id) }}" method="POST">
                                    @csrf @method('DELETE')
                                    <button type="button" @click="confirmDeleteFAQ($event)" 
                    ...
                    @empty
                    <tr>
                        <td colspan="3" class="px-10 py-16 text-center text-slate-400 italic font-medium">Belum ada FAQ yang tersedia.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
<script>
    function faqManager() {
        return {
            ...
            editMode: false,
            actionUrl: '',
            formData: { pertanyaan: '', jawaban: '', status: '1' },

            openTambah() {
                this.editMode = false;
                this.formData = { pertanyaan: '', jawaban: '', status: '1' };
                this.actionUrl = '{{ route("faq.store") }}';
                this.openModal = true;
            },
            ...
            openEdit(item) {
                ...
                this.actionUrl = `/management/faq/${item.id}`; 
                this.openModal = true;
            },
            ...
            confirmDeleteFAQ(e) {
                ...
                Swal.fire({
                    title: 'Hapus FAQ ini?',
                    text: "Pertanyaan ini tidak akan muncul lagi di halaman publik.",
                    icon: 'warning',
