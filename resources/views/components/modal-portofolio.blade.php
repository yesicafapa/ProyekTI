                ...
                <div class="flex items-center gap-5">
                    <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-orange-500 text-white shadow-lg shadow-orange-500/20">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M20.25 14.15v4.25c0 .414-.336.75-.75.75H4.5a.75.75 0 0 1-.75-.75v-4.25m16.5 0a3 3 0 0 0-3-3H6.75a3 3 0 0 0-3 3m16.5 0V10.5a2.25 2.25 0 0 0-2.25-2.25h-1.5a1.125 1.125 0 0 1-1.125-1.125V4.875A2.625 2.625 0 0 0 12.75 2.25h-1.5a2.625 2.625 0 0 0-2.625 2.625V7.125c0 .622-.504 1.125-1.125 1.125h-1.5a2.25 2.25 0 0 0-2.25 2.25v3.65m13.5 0v3.75m-9-3.75v3.75M9 3.75h6" />
                        </svg>
                    </div>
                    <div>
                        <h3 class="text-2xl font-black italic tracking-tighter text-slate-800 uppercase dark:text-white" x-text="editMode ? 'EDIT PORTOFOLIO' : 'TAMBAH PORTOFOLIO'"></h3>
                        <p class="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-gray-500">Pamerkan karya terbaik CV SEOVDETECH</p>
                    </div>
                </div>
                ...
                <form :action="actionUrl" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 gap-8 md:grid-cols-12">
                        ...
                        <div class="space-y-6 md:col-span-7">
                            <div>
                                <label class="mb-2 block text-[10px] font-black uppercase tracking-widest text-orange-500/80">Nama Project</label>
                                <input type="text" name="judul" x-model="formData.judul" placeholder="Contoh: Aplikasi E-Commerce..." required 
                            ...
                            <div>
                                <label class="mb-2 block text-[10px] font-black uppercase tracking-widest text-orange-500/80">URL / Link Demo</label>
                                <input type="url" name="url" x-model="formData.url" placeholder="https://..." 
                                    class="w-full rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm font-bold text-slate-800 outline-none focus:border-orange-500 transition-all dark:border-white/10 dark:bg-white/5 dark:text-white">
                            </div>

                            <div>
                                <label class="mb-2 block text-[10px] font-black uppercase tracking-widest text-orange-500/80">Deskripsi Project</label>
                                <textarea name="deskripsi" x-model="formData.deskripsi" rows="6" placeholder="Jelaskan tentang project ini..." required 
                        ...
                        <div class="space-y-6 md:col-span-5">
                            <div>
                                ...
                                <div class="relative group">
                                    <select name="status" x-model="formData.status" 
                                        ...
                                        <option value="1" class="text-slate-900 bg-white">TAMPILKAN DI WEB</option>
                            ...
                            <div>
                                <label class="mb-2 block text-[10px] font-black uppercase tracking-widest text-orange-500/80">Screenshot Project</label>
                                <div class="group relative aspect-video w-full overflow-hidden rounded-[2rem] border-2 border-dashed border-slate-200 bg-slate-50 hover:border-orange-500 transition-all dark:border-white/10 dark:bg-white/5">
                                    ...
                                    <div x-show="!imageUrl" class="flex h-full flex-col items-center justify-center text-slate-400 dark:text-gray-500">
                                        ...
                                        <span class="text-[9px] font-black uppercase tracking-[0.2em]">Upload Screenshot</span>
                                    </div>
                                    <input type="file" name="gambar" class="absolute inset-0 cursor-pointer opacity-0" @change="const file = $event.target.files[0]; if (file) { imageUrl = URL.createObjectURL(file); }">
                                </div>
                            </div>
                        </div>
                    </div>
