        ...
        <div x-show="openModal"
            ...
            <div class="flex items-center justify-between border-b border-slate-100 bg-white px-10 py-8 dark:border-slate-800 dark:bg-[#1a2233] rounded-t-[2.5rem]">
                <div class="flex items-center gap-5">
                    ...
                    <div>
                        <h3 class="text-2xl font-black tracking-tight text-slate-800 dark:text-white" x-text="editMode ? 'Edit Testimoni' : 'Tambah Testimoni Baru'"></h3>
                        <p class="text-xs font-medium text-slate-400">Feedback berharga dari klien CV SEOVDETECH.</p>
                    </div>
                </div>
                ...
                <form :action="actionUrl" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 gap-10 md:grid-cols-12">
                        ...
                        <div class="space-y-6 md:col-span-7">
                            <div>
                                <label class="mb-2 block text-[10px] font-black uppercase tracking-widest text-slate-400">Nama Klien / Pengguna</label>
                            ...
                            <div>
                                <label class="mb-2 block text-[10px] font-black uppercase tracking-widest text-slate-400">Isi Testimoni</label>
                        ...
                        <div class="space-y-6 md:col-span-5">
                            <div>
                                <label class="mb-2 block text-[10px] font-black uppercase tracking-widest text-slate-400">Status Tampilan</label>
                                <select name="status" 
                                    x-model="formData.status" 
                                        class="w-full rounded-xl border-slate-200 bg-white px-4 py-3 text-sm font-medium outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 dark:border-slate-700 dark:bg-[#1a2233] dark:text-white transition-all"
                                        >
                                        <option value="1" class="dark:bg-[#1a2233]">PUBLIKASIKAN</option>
                                        <option value="0" class="dark:bg-[#1a2233]">SIMPAN DRAFT</option>
                                </select>
                            </div>

                            <div>
                                <label class="mb-2 block text-[10px] font-black uppercase tracking-widest text-slate-400">Foto Profil Klien</label>
                                <div class="group relative aspect-square w-full max-w-[240px] mx-auto overflow-hidden rounded-[2.5rem] border-2 border-dashed border-slate-200 bg-slate-50 hover:border-orange-500 transition-all dark:border-slate-700 dark:bg-slate-800/50">
                                    ...
                                    <div x-show="!imageUrl" class="flex h-full flex-col items-center justify-center text-slate-400">
                                        ...
                                        <span class="text-[10px] font-black uppercase tracking-widest">Upload Foto</span>
                                    </div>

                                    <input type="file" name="image_pengguna" class="absolute inset-0 cursor-pointer opacity-0" 
                                        @change="const file = $event.target.files[0]; if (file) imageUrl = URL.createObjectURL(file)">
                                </div>
                                <p class="mt-3 text-center text-[10px] font-medium text-slate-400 uppercase tracking-tighter">*Rekomendasi ukuran 1:1 (Square)</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-12 flex items-center justify-end gap-6 border-t border-slate-100 pt-10 dark:border-slate-800">
                        ...
                        <button type="submit" class="rounded-2xl bg-orange-500 px-14 py-5 text-xs font-black uppercase tracking-[0.2em] text-white shadow-2xl shadow-orange-500/40 hover:bg-orange-600 active:scale-95 transition-all">
                            <span x-text="editMode ? 'Update Testimoni' : 'Simpan Testimoni'"></span>
                        </button>
                    </div>
                </form>
            </div>
